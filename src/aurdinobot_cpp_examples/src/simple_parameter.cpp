...
public:
...

    {
        ...
        declare_parameter<std::string> ("simple_string_param", "Rishi");
...
private:
    ...
    rcl_interfaces::msg::SetParametersResult paramchangeCallback(const std::vector<rclcpp::Parameter> &parameters)
    {
        ...
        for(const auto& param : parameters)
        {
            ...
            {
                RCLCPP_INFO_STREAM(get_logger(), "Parameter simple_string_param changed New vlaue is : " << param.as_string());
                result.successful = true;
            }
